...
const SelectCustomInput = ({data, allow, onChange}) => {
    ...
    return (
        <div className="w-full">
            <p className="text-lg font-bold text-gray-800 dark:text-white">Tags</p>
            <div className="flex gap-[.5em] flex-wrap py-2">
                {data.length === 0 && (
                    <div
                    className="flex items-center gap-1 w-max bg-slate-200 dark:bg-dark border-[1px] border-slate-200 rounded px-1.5 py-0.5 text-gray-800 dark:text-white">
                        <span className="relative top-0.5">tags input in here</span>
                    </div>
                )}
                {data.map((tag, index) => (
                    <div
